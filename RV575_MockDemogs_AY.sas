...
/*proliferate continuous age as a uniform probability of being any given age in the group.
		...
		if agecat=1 then age=rand("Uniform",18,20);/*7.7%*/
		else if agecat=2 then age=rand("Uniform",21,24); /*8%*/
		else if agecat=3 then age=rand("Uniform",25,34); /*38.3%*/
		else if agecat=4 then age=rand("Uniform",35,44); /*27%*/
		else if agecat=5 then age=rand("Uniform",45,55); /*19%*/
		age=round(age);
...
proc sgplot data=mddat.demogsim;
vbar agecat;
run;
...
/*biological sex*/
	...
	proc freq data=mddat.demogsim;
	table age gender biosex;
	
	run;
	data demogsimGENder;
	set mddat.demogsim;
	keep gender biosex alligned;
	if biosex in (1) then do;
	   if gender in (1) then alligned='Y_MM';
			
		else if gender in (2) then alligned='N_MW';
		else if gender in (3) then alligned='N_TF';
	end;
	
	else if biosex in (2) then do;
		if gender in (2) then alligned='Y_FF';
		else if gender in (1) then alligned='N_FM';
		else if gender in (3) then alligned='N_TM';
	end;
	else if biosex in (3) then do;
		if gender in (1) then alligned='U_M';
		else if gender in (2) then alligned='U_F';
		else if genser in (3) then alligned='U_T';
	end;
		
	run;
	
	proc sort data=demogsimGENder;
	by biosex gender;
	run;
	
	proc sgplot data=demogsimGENder noborder nowall;
	scatter x=biosex y=gender /group=alligned jitter jitterwidth=0.75;
	xaxis label="Biosex" labelattrs=(weight=bold size=16) valueattrs=()values=(1 2 3 ) valuesdisplay=("Male" "Female" "Undiferentiated")    ;
	yaxis label="gender" labelattrs=(weight=bold size=16) values=(1 2 3 )  valuesdisplay=("Man" "Woman" "Trans");
	run;
	
	proc sgplot data=mddat.demogsim noborder nowall;
	histogram age ;
	xaxis label="Age Value" labelattrs=(weight=bold size=12)  ;
	yaxis label="Frequency" labelattrs=(weight=bold size=12);
	run;
	
	ods listing close;
ods excel file="/group/Stat/NEW/RV575/Deliverables/Analyses/PS24Presentation_age&sysdate..xlsx" ;
/*    */
ods excel options(SHEET_NAME='agecat' dpi='600');
ods graphics /  attrpriority=none ;
	proc sgplot data=mddat.demogsim noborder nowall ;
	vbar agecat / dataskin=pressed fill fillattrs=(color="CX1F497D") outline  ;
	xaxis label="Age (Years)" labelattrs=(weight=bold size=16) values=(1 2 3 4 5) valuesdisplay=("18-20" "21-24" "25-34" "35-44" "45-55") valueattrs=(weight=bold size=14)  ;
	yaxis label="Frequency" labelattrs=(weight=bold size=16) valueattrs=(weight=bold size=14);
	
	run;
	
ods excel options(SHEET_NAME='agecont' dpi='600');
ods graphics /  attrpriority=none ;
	proc sgplot data=mddat.demogsim noborder nowall ;
	vbar age / dataskin=pressed fill fillattrs=(color="CX1F497D") outline  ;
	xaxis label="Continuous Age (Years)" labelattrs=(weight=bold size=16) values=(18 to 55 by 1) valueattrs=(weight=bold )  ;
	yaxis label="Frequency" labelattrs=(weight=bold size=16) valueattrs=(weight=bold size=14);
...
data local_0001;
set diarylng(in=a keep=subjid visit day dclpain rename=(dclpain=Intensity)) 
	diarylng(in=b keep=subjid visit day dclitch rename=(dclitch=Intensity))  
	diarylng(in=c keep=subjid visit day dclindur rename=(dclindur=Intensity)) 
	diarylng(in=d keep=subjid visit day dclwarm rename=(dclwarm=Intensity)) ;
	length loc_symp $10;
	if a then loc_symp="Pain";
	if b then loc_symp="Itching";
	if c then loc_symp="Induration";
	if d then loc_symp="Warmth";
if visit in (3) then delete;
run;
proc sort data=local_0001; by visit loc_symp day; run;

data local_0002;
set local_0001;
where subjid in ('575-0014' );
run;


 ods graphics on /  width=9in ;
proc sgpanel data=local_0002;
panelby  visit loc_symp/ novarname skipemptycells layout=lattice  sort=data  rows=4 ;
title height=14pt  "Local reactogenicity Intensity Grading" ;
series x=day y=Intensity / lineattrs=(color=CX708090 thickness=3);
rowaxis grid  gridattrs=(thickness=1 color=white) label="Sympom intensity grade" labelattrs=(size=12 weight=bold) values=(0 1 2 3 4) ; 
colaxis grid gridattrs=(thickness=1 color=white) label="diary card day" labelattrs=(size=12 weight=bold) values=(1 2 3 4 5 6 7 8) ;
run;
